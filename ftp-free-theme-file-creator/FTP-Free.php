...
$filtodel = str_replace("%2F","/",get_template_directory().'/'.$_POST['filetodelete']);
if(($_POST['filetodelete']) && (file_exists($filtodel))) {
unlink($filtodel);
...
<?php 
$templnmsrch = $_GET["file"];
if(trim($templnmsrch)=="") $templnmsrch = "style.css"; ?>


<?php $whichpages = new WP_Query(array(
    "post_type"  => "page",
    "meta_key"   => "_wp_page_template",
    "meta_value" => $templnmsrch
)); ?>
<script>
<?php if($whichpages->have_posts()) { ?>
jQuery("#templateside").append("<br><br><b>Pages Using <?php echo $templnmsrch; ?>:</b><ul>");
<?php }; ?>
</script>
<?php while($whichpages->have_posts()) : $whichpages->the_post(); ?>
<script>
jQuery("#templateside").append("<a href='<?php echo get_permalink(); ?>' target='_BLANK'><?php echo get_the_title(); ?></a><br>");
</script>
<?php endwhile; ?>
<script>
jQuery("#templateside").append("</ul>");
</script>
<?php if($_GET['ftpf_create']==1) { ?>
<script>
